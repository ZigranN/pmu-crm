...
ALTER TABLE "services" ADD CONSTRAINT "services_definition_sessions_fk" FOREIGN KEY ("catalog_code","sessions_model") REFERENCES "public"."service_definitions"("code","sessions_model") ON DELETE no action ON UPDATE no action;--> statement-breakpointCREATE UNIQUE INDEX "services_id_studio_unique" ON "services" USING btree ("id","studio_id");--> statement-breakpointCREATE UNIQUE INDEX "whatsapp_templates_id_studio_unique" ON "whatsapp_templates" USING btree ("id","studio_id");--> statement-breakpoint
