...
export async function searchClientsAction(studioId: string, query: string) {
  return sensitiveRead("CLIENT_READ", studioId, { operation: "clients.search" }, async (context) => {
...
export async function globalSearchAction(studioId: string, query: string) {
  return sensitiveRead(null, studioId, { operation: "global.search" }, async (context) => {
    const scope = await resourceScope(context);
  ...
  }, result => [...recordIds(result.clients), ...recordIds(result.masters), ...recordIds(result.services)]);
}
