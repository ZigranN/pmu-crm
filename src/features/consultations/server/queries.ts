...
export async function getConsultationPanel(id:string) {
  ...
  return sensitiveRead("MEDICAL_PROFILE_READ",undefined,{operation:"consultations.read",targetId:id},context=>db.transaction(async tx=>{
    ...
    const sessions=await tx.select().from(consultations).where(and(eq(consultations.cycleId,id),eq(consultations.studioId,context.studioId))).orderBy(desc(consultations.completedAt),desc(consultations.id)).limit(100);
