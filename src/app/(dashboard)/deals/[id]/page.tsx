...
export default async function DealPage({params}:{params:Promise<{id:string}>}) {
 ...
 const terms=await hasPermission(db,context.userId,context.studioId,"PAYMENT_READ")?await getCommercialTerms(id):null;
 return <div className="space-y-8"><CycleTimeline data={timeline}/>{terms&&<TermsPanel data={terms}/>}{panel&&<ConsultationPanel data={panel}/>}</div>;
}
