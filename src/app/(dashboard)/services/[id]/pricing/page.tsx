...
export default async function PricingPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params, options = await getServicePricing(id);
  if (!options.length) notFound();
  return <div className="space-y-6"><PageHeader title="Цены мастеров" description={options[0].serviceName} backHref={`/services/${id}/edit`} />
